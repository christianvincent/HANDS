...
namespace Glitch9.Editor
{
    public class JavaSyntaxHighlighter : SyntaxHighlighter
    {
        protected override string HighlightInternal(string code)
        {
            ...
            // Pinks: return, if, else, while, for, foreach, break, continue, switch, case, default, try, catch, throw
            code = Regex.Replace(code, @"\b(return|if|else|while|for|foreach|break|continue|switch|case|default|try|catch|throw)\b", m => $"<color={Colors.Pink}>{m.Value}</color>");
